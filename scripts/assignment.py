...
def sobel_edge_detection(filepath):
	...
	for row in xrange(0,new.shape[0]):
		for column in xrange(0,new.shape[1]):
			if new[row, column] > 30:
				new[row, column] = 255
			else :
				new[row, column] = 0
...
def sobel_for_canny_edge_detection(filepath):
	img = cv2.imread(filepath, cv2.CV_LOAD_IMAGE_GRAYSCALE)
	padded_image = cv2.copyMakeBorder(img,1,1,1,1,cv2.BORDER_CONSTANT,value = 0)
	Gximg = img.copy()
	Gyimg = img.copy()
...
def canny_edge_detection(filepath, t1, t2):
	...
	sobel_result = sobel_for_canny_edge_detection('..\\results\\gaussian_filter.png')

	canny = sobel_result.copy()

	for row in xrange(0,sobel_result.shape[0]):
		for column in xrange(0,sobel_result.shape[1]):
			if row == 0 or row == canny.shape[0] - 1 or column == 0 or column == canny.shape[1] - 1 or canny[row, column] < t2:
				canny[row, column] = 0
			if sobel_result[row, column] > t1:
					canny[row, column] = 255
			else :
				if sobel_result[row, column] < t2:
					canny[row, column] = 0
	...
	for row in xrange(0,canny.shape[0]):
		for column in xrange(0,canny.shape[1]):
			if canny[row, column] != 0 and canny[row, column] != 255:
				# print canny[row,column]
				if canny[row - 1, column - 1] == 255 or canny[row - 1, column] == 255 or canny[row - 1, column + 1] == 255 or canny[row, column - 1] == 255 or canny[row, column + 1] == 255 or canny[row + 1, column - 1] == 255 or canny[row + 1, column] == 255 or canny[row + 1, column + 1] == 255 :
					canny[row, column] = 255
				else :
					canny[row, column] = 0
				
	os.remove("..\\results\\gaussian_filter.png")
...
canny_edge_detection(image2, 40, 20)
